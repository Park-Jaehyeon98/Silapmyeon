...
const InterviewCarousel = ({ interviews }) => {
  ...
  const prevInterview = () => {
    if (index - 5 >= 0) { // 수정: 마지막 5개 아이템에 도달하면 더 이상 증가시키지 않음
        setIndex(index - 5);
      }
      ...
      <div className={styles.carouselBox}>
        <div style={transformStyle}>
            ...
            <div key={i} className={styles.interviewItem}>
                ...
                <div className={styles.interviewItemChild}>{formatDate(interview.date)}</div>
                <div className={styles.interviewItemChild}>{interview.company}</div>
            </div>
            ))}
        </div>
      </div>
